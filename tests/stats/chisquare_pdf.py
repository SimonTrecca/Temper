#!/usr/bin/env python3
"""
Generator for chisquare::pdf tests (random-grid + broadcasting + safe view-with-weird-strides).
"""
...
def create_test_chi2(dof, n):
    ...
    lines = []
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.pdf_random_test_dof_{dof}')
    lines.append(f'  * @brief Randomly generated chisquare pdf test on {n} matrix with dof = {dof}.')
    lines.append('  *')
    ...
    x = np.linspace(0.0, 10.0 + 3*dof, n)
    ...
    lines.append('    // run Temper chisquare::pdf')
    lines.append('    Tensor<float> result = stats::chisquare::pdf(x, k);')
...
def make_broadcast_test_block(testname, x_shape, x_vals, k_shape, k_vals):
    ...
    expected = stats.chi2.pdf(out_b, k_b)
...
def make_view_strides_chi2_test(testname,
    ...
    expected_np = stats.chi2.pdf(x_alias_np, k_alias_np)
    ...
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.{testname}')
    lines.append('  * @brief Ensure chisquare::pdf accepts non-contiguous/alias views for x and k')
    lines.append('  */')
    lines.append(f'TEST(CHISQUARE, {testname})')
    lines.append('{')
    ...
    lines.append('    Tensor<float> result = stats::chisquare::pdf(x_alias, k_alias);')
    lines.append('')
...
if __name__ == '__main__':
  ...
  */""")
    ...
    k1_shape = (1,)
    k1_vals = [3.0]
    bc.append(make_broadcast_test_block('pdf_broadcast_x_vector_k_scalar', x1_shape, x1_vals, k1_shape, k1_vals))

    x2_shape = (1,)
    x2_vals = [1.0]
    k2_shape = (3,)
    k2_vals = [3.0, 4.0, 2.0]
    bc.append(make_broadcast_test_block('pdf_broadcast_x_scalar_k_vector', x2_shape, x2_vals, k2_shape, k2_vals))

    x3_shape = (2, 2)
    x3_vals = [0.0, 1.0, 2.0, 3.0]
    k3_shape = (2,)
    k3_vals = [3.0, 2.0]
    bc.append(make_broadcast_test_block('pdf_broadcast_2d_x_k_vector', x3_shape, x3_vals, k3_shape, k3_vals))

    x4_shape = (2, 1)
    x4_vals = [0.0, 1.0]
    k4_shape = (1, 3)
    k4_vals = [2.0, 3.0, 4.0]
    bc.append(make_broadcast_test_block('pdf_broadcast_mixed_shapes_all_operands', x4_shape, x4_vals, k4_shape, k4_vals))
