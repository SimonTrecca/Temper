#!/usr/bin/env python3
"""
Generator for chisquare::logpdf tests (random-grid + broadcasting + safe view-with-weird-strides).
...
# Small positive value to avoid exact zeros (which can produce +/-inf in logpdf)
EPS = 1e-6
...
def create_test_chi2(dof, n):
    ...
    lines = []
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.logpdf_random_test_dof_{dof}')
    lines.append(f'  * @brief Randomly generated chisquare logpdf test on {n} matrix with dof = {dof}.')
    lines.append('  *')
    ...
    # start at EPS instead of 0.0 to avoid +/-inf for some dof values
    x = np.linspace(EPS, 10.0 + 3*dof, n)
    y = stats.chi2.logpdf(x, dof)
    ...
    lines.append('    // run Temper chisquare::logpdf')
    lines.append('    Tensor<float> result = stats::chisquare::logpdf(x, k);')
...
def make_broadcast_test_block(testname, x_shape, x_vals, k_shape, k_vals):
    # replace zeros in x_vals with EPS to avoid infinity in logpdf
    x_vals_safe = [EPS if (isinstance(v, (int, float)) and v == 0.0) else v for v in x_vals]

    out_shape = broadcast_shape(x_shape, k_shape)
    x_np = np.asarray(x_vals_safe).reshape(x_shape)
    k_np = np.asarray(k_vals).reshape(k_shape)
    out_b = np.broadcast_to(x_np, out_shape)
    k_b = np.broadcast_to(k_np, out_shape)
    expected = stats.chi2.logpdf(out_b, k_b)
    ...
    lines.append('    ' + cpp_vec_or_scalar_assignment('x', np.asarray(x_vals_safe).ravel(), x_shape))
...
def make_view_strides_chi2_test(testname,
    ...
    # ensure owner arrays don't have zeros in x positions that will be used
    x_owner_np = np.asarray([EPS if (isinstance(v, (int, float)) and v == 0.0) else v for v in x_owner_vals], dtype=float)
    ...
    expected_np = stats.chi2.logpdf(x_alias_np, k_alias_np)
    ...
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.{testname}')
    lines.append('  * @brief Ensure chisquare::logpdf accepts non-contiguous/alias views for x and k')
    lines.append('  */')
    lines.append(f'TEST(CHISQUARE, {testname})')
    lines.append('{')

    lines.append(f'    Tensor<float> x_owner({{{len(x_owner_vals)}}}, {memloc});')
    xv_items = ', '.join(fmt_float(float(v)) for v in x_owner_np)
    ...
    lines.append('    Tensor<float> result = stats::chisquare::logpdf(x_alias, k_alias);')
    lines.append('')
...
if __name__ == '__main__':
  ...
  */""")
    ...
    x1_shape = (4,)
    # replaced zeros with EPS to avoid +/-inf
    x1_vals = [EPS, EPS, 0.5, 2.0]
    k1_shape = (1,)
    k1_vals = [3.0]
    bc.append(make_broadcast_test_block('logpdf_broadcast_x_vector_k_scalar', x1_shape, x1_vals, k1_shape, k1_vals))

    x2_shape = (1,)
    x2_vals = [1.0]
    k2_shape = (3,)
    k2_vals = [3.0, 4.0, 2.0]
    bc.append(make_broadcast_test_block('logpdf_broadcast_x_scalar_k_vector', x2_shape, x2_vals, k2_shape, k2_vals))

    x3_shape = (2, 2)
    # replaced leading 0.0 with EPS
    x3_vals = [EPS, 1.0, 2.0, 3.0]
    k3_shape = (2,)
    k3_vals = [3.0, 2.0]
    bc.append(make_broadcast_test_block('logpdf_broadcast_2d_x_k_vector', x3_shape, x3_vals, k3_shape, k3_vals))

    x4_shape = (2, 1)
    # replaced 0.0 with EPS
    x4_vals = [EPS, 1.0]
    k4_shape = (1, 3)
    k4_vals = [2.0, 3.0, 4.0]
    bc.append(make_broadcast_test_block('logpdf_broadcast_mixed_shapes_all_operands', x4_shape, x4_vals, k4_shape, k4_vals))
    ...
    blocks.append(make_view_strides_chi2_test(
        'logpdf_view_with_weird_strides',
        x_owner_vals, x_offset, x_alias_shape, x_alias_strides,
        k_owner_vals, k_offset, k_alias_shape, k_alias_strides,
        memloc='MemoryLocation::DEVICE',
        tol=1e-6
