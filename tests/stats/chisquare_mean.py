#!/usr/bin/env python3
"""
Generator for stats::chisquare:mean tests validated against scipy.stats.chi2.mean.
...
def make_mean_test(testname, k_shape, k_vals):
    """
    Build a test that constructs k, computes expected = scipy.stats.chi2.mean(k),
    and checks stats::chisquare::mean(k) elementwise against expected.
    """
    k_np = np.asarray(k_vals).reshape(k_shape)
    # SciPy returns array-like when passed array-like
    expected_np = stats.chi2.mean(k_np)
    ...
    lines.append(f'  * @brief Verify stats::chisquare::mean matches scipy.stats.chi2.mean for {testname}')
    lines.append('  */')
    lines.append(f'TEST(CHISQUARE, {testname})')
    lines.append('{')
    ...
    lines.append('')
    # call
    lines.append('    Tensor<float> result = stats::chisquare::mean(k);')
    lines.append('')
    # expected_result
...
def make_view_strides_mean_test(testname,
    ...
    expected_np = stats.chi2.mean(k_alias_np)
    ...
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.{testname}')
    lines.append('  * @brief Ensure stats::chisquare::mean accepts non-contiguous/alias views for k and matches SciPy')
    lines.append('  */')
    lines.append(f'TEST(CHISQUARE, {testname})')
    lines.append('{')
    ...
    lines.append('    Tensor<float> result = stats::chisquare::mean(k_alias);')
    lines.append('')
...
if __name__ == '__main__':
  ...
  */""")
    ...
    blocks.append(make_mean_test('mean_scalar_df_3', (1,), [3.0]))

    # vector of dfs
    blocks.append(make_mean_test('mean_vector_dfs', (4,), [1.0, 2.0, 3.0, 4.0]))

    # random vector of dfs
    r = np.abs(np.random.randn(6)) + 0.1  # ensure positive df
    blocks.append(make_mean_test('mean_vector_random_positive', (6,), r.tolist()))

    # 2D shape (matrix of dfs)
    blocks.append(make_mean_test('mean_matrix_2x3', (2,3),
    ...
    k_alias_shape = (3,)
    k_alias_strides = (2,)
    blocks.append(make_view_strides_mean_test(
        'mean_view_with_weird_strides',
        k_owner_vals, k_offset, k_alias_shape, k_alias_strides,
        memloc='MemoryLocation::DEVICE',
        tol=tolerance
    ))
