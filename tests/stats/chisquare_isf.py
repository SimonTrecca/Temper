#!/usr/bin/env python3
"""
Generator for chisquare::isf tests (random-grid + broadcasting + safe view-with-weird-strides).
"""
...
def create_test_chi2(dof, n):
    ...
    lines = []
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.isf_random_test_dof_{dof}')
    lines.append(f'  * @brief Randomly generated chisquare isf test on {n} matrix with dof = {dof}.')
    lines.append('  *')
    ...
    lines.append('    // run Temper chisquare::isf')
    lines.append('    Tensor<float> result = stats::chisquare::isf(q, k);')
...
def make_broadcast_test_block(testname, q_shape, q_vals, k_shape, k_vals):
    ...
    expected = stats.chi2.isf(out_b, k_b)
...
def make_view_strides_chi2_test(testname,
    ...
    expected_np = stats.chi2.isf(q_alias_np, k_alias_np)
    ...
    lines.append('/**')
    lines.append(f'  * @test CHISQUARE.{testname}')
    lines.append('  * @brief Ensure chisquare::isf accepts non-contiguous/alias views for x and k')
    lines.append('  */')
    lines.append(f'TEST(CHISQUARE, {testname})')
    lines.append('{')
    ...
    lines.append('    Tensor<float> result = stats::chisquare::isf(q_alias, k_alias);')
    lines.append('')
...
if __name__ == '__main__':
  ...
  */""")
    ...
    k1_shape = (1,)
    k1_vals = [3.0]
    bc.append(make_broadcast_test_block('isf_broadcast_q_vector_k_scalar', q1_shape, q1_vals, k1_shape, k1_vals))

    q2_shape = (1,)
    q2_vals = [0.5]
    k2_shape = (3,)
    k2_vals = [3.0, 4.0, 2.0]
    bc.append(make_broadcast_test_block('isf_broadcast_q_scalar_k_vector', q2_shape, q2_vals, k2_shape, k2_vals))

    q3_shape = (2, 2)
    q3_vals = [0.05, 0.25, 0.5, 0.95]
    k3_shape = (2,)
    k3_vals = [3.0, 2.0]
    bc.append(make_broadcast_test_block('isf_broadcast_2d_q_k_vector', q3_shape, q3_vals, k3_shape, k3_vals))

    x4_shape = (2, 1)
    x4_vals = [0.33, 0.66]
    k4_shape = (1, 3)
    k4_vals = [2.0, 3.0, 4.0]
    bc.append(make_broadcast_test_block('isf_broadcast_mixed_shapes_all_operands', x4_shape, x4_vals, k4_shape, k4_vals))
